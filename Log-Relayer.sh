...
while read -r line
do
     echo -en "    "; echo -en $i; echo -en ") "; cat device-ID | grep -i $line                   ##Mapping ip to device ID
     i=$(( i + 1 ))
done < "$filename"
echo
echo
...
#Enough introduction now lets talk buisness' >> worker.sh
while read -r line
do
     echo -en 'send -- {' >> worker.sh; echo -en $line >> worker.sh; echo '}' >> worker.sh    ## dont mess with BELOW SPACING !!
     echo 'send -- "\r"
expect "#"   
send -- "\r"
expect "#"' >> worker.sh
...
do
     ...
     echo "########################################################################" >> $logging
     echo >> $logging
     ./worker.sh $device $myidentity $myprecious >> $logging 2>&1
     echo >> $logging
     echo >> $logging 
     cat $logging
